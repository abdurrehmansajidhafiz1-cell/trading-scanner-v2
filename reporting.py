...
def generate_instant_signal_alert_text(z: dict) -> str:
    ...
    lines = [
        ...
        "--------------------------------------------------------------------------------",
        f"Tier 1 Entry (61.8% Fib):       {_fmt_num(p_e1)}  [Allocate 50% Size]",
        f"Tier 2 Entry (78.6% OTE):       {_fmt_num(p_e2)}  [Allocate 50% Size]",
        ...
        "",
        "================================================================================",
        f"💵 PLAYBOOK 1: $100 CAPITAL (USD) — AFTER BINANCE FEES (@ {fee_rate*100:.3f}%):",
        "================================================================================",
        "",
        ...
        "",
        "================================================================================",
        f"🇵🇰 PLAYBOOK 2: $35 CAPITAL (IN PAKISTANI RUPEES — PKR @ Rs. {pkr_rate:,.0f}/USDT):",
        "================================================================================",
        f"Initial Capital: Rs. {35.0*pkr_rate:,.2f} PKR ($35.00 USDT) | Tier 1: Rs. {17.5*pkr_rate:,.2f} | Tier 2: Rs. {17.5*pkr_rate:,.2f}",
        "",
        "[SCENARIO 1] Full Fill (Rs. 4,900 @ 61.8 + Rs. 4,900 @ 78.6) -> 100% Exit at TP1",
        f"  • Avg Entry: {_fmt_num(avg_entry_full)} ({tot_coins_full*0.35:.4f} coins)",
        f"  • Gross: Rs. {gross_s1*0.35*pkr_rate:+,.2f} PKR | Fees: -Rs. {fee_s1*0.35*pkr_rate:,.2f} PKR",
        f"  • NET PROFIT: Rs. {net_s1*0.35*pkr_rate:+,.2f} PKR ({net_s1:+.2f}%)  --> Wallet: Rs. {(35.0+net_s1*0.35)*pkr_rate:,.2f} PKR",
        "",
        "[SCENARIO 2] Shallow Reversal (Only 61.8% Fill) -> 100% Exit at TP1",
        f"  • Entry: {_fmt_num(p_e1)} ({c_shallow*0.35:.4f} coins - Rs. {17.5*pkr_rate:,.2f} Capital)",
        f"  • Gross: Rs. {gross_s2*0.35*pkr_rate:+,.2f} PKR | Fees: -Rs. {fee_s2*0.35*pkr_rate:,.2f} PKR",
        f"  • NET PROFIT: Rs. {net_s2*0.35*pkr_rate:+,.2f} PKR ({net_s2:+.2f}%)  --> Wallet: Rs. {(35.0+net_s2*0.35)*pkr_rate:,.2f} PKR",
        "",
        f"[SCENARIO 3] Full Fill -> 50% Exit @ BE ({_fmt_num(p_be)}) + 50% Exit @ TP1 ({_fmt_num(p_tp1)})",
        f"  • 50% locked at BE move + 50% at full target",
        f"  • Gross: Rs. {gross_s3*0.35*pkr_rate:+,.2f} PKR | Fees: -Rs. {fee_s3*0.35*pkr_rate:,.2f} PKR",
        f"  • NET PROFIT: Rs. {net_s3*0.35*pkr_rate:+,.2f} PKR ({net_s3:+.2f}%)  --> Wallet: Rs. {(35.0+net_s3*0.35)*pkr_rate:,.2f} PKR",
        "",
        f"[SCENARIO 4] Shallow Fill -> 50% Exit @ BE ({_fmt_num(p_be)}) + 50% Exit @ TP1 ({_fmt_num(p_tp1)})",
        f"  • Only 61.8% touched, Rs. {17.5*pkr_rate:,.2f} cash remains safe in wallet",
        f"  • Gross: Rs. {gross_s4*0.35*pkr_rate:+,.2f} PKR | Fees: -Rs. {fee_s4*0.35*pkr_rate:,.2f} PKR",
        f"  • NET PROFIT: Rs. {net_s4*0.35*pkr_rate:+,.2f} PKR ({net_s4:+.2f}%)  --> Wallet: Rs. {(35.0+net_s4*0.35)*pkr_rate:,.2f} PKR",
        "",
        "[SCENARIO 5] Full Fill -> 70% Exit @ TP1 + 30% Exit @ TP2 (Runner Mode)",
        f"  • Gross: Rs. {gross_s5*0.35*pkr_rate:+,.2f} PKR | Fees: -Rs. {fee_s5*0.35*pkr_rate:,.2f} PKR",
        f"  • NET PROFIT: Rs. {net_s5*0.35*pkr_rate:+,.2f} PKR ({net_s5:+.2f}%)  --> Wallet: Rs. {(35.0+net_s5*0.35)*pkr_rate:,.2f} PKR",
        "",
        "[SCENARIO 6] Shallow Fill -> 70% Exit @ TP1 + 30% Exit @ TP2 (Runner Mode)",
        f"  • Gross: Rs. {gross_s6*0.35*pkr_rate:+,.2f} PKR | Fees: -Rs. {fee_s6*0.35*pkr_rate:,.2f} PKR",
        f"  • NET PROFIT: Rs. {net_s6*0.35*pkr_rate:+,.2f} PKR ({net_s6:+.2f}%)  --> Wallet: Rs. {(35.0+net_s6*0.35)*pkr_rate:,.2f} PKR",
        "",
        f"[SCENARIO 7] Full Fill -> 50% Exit @ BE ({_fmt_num(p_be)}) -> Retrace to Avg Entry SL",
        f"  • Market hit 55% BE, locked 50% gain, then returned to Entry (Risk-Free Exit)",
        f"  • Gross: Rs. {gross_s7*0.35*pkr_rate:+,.2f} PKR | Fees: -Rs. {fee_s7*0.35*pkr_rate:,.2f} PKR",
        f"  • NET PROFIT: Rs. {net_s7*0.35*pkr_rate:+,.2f} PKR ({net_s7:+.2f}%)  --> Wallet: Rs. {(35.0+net_s7*0.35)*pkr_rate:,.2f} PKR",
        "",
        f"[SCENARIO 8] Shallow Fill -> 50% Exit @ BE ({_fmt_num(p_be)}) -> Retrace to Entry SL",
        f"  • Only 61.8% touched, locked 50% at BE, then returned to Entry (Risk-Free Exit)",
        f"  • Gross: Rs. {gross_s8*0.35*pkr_rate:+,.2f} PKR | Fees: -Rs. {fee_s8*0.35*pkr_rate:,.2f} PKR",
        f"  • NET PROFIT: Rs. {net_s8*0.35*pkr_rate:+,.2f} PKR ({net_s8:+.2f}%)  --> Wallet: Rs. {(35.0+net_s8*0.35)*pkr_rate:,.2f} PKR",
        "",
        f"[SCENARIO 9] WORST CASE: Full Fill (Rs. {35.0*pkr_rate:,.2f}) -> Direct Stop Loss Hit ({_fmt_num(p_sl)})",
        f"  • Gross: Rs. {gross_s9*0.35*pkr_rate:+,.2f} PKR | Fees: -Rs. {fee_s9*0.35*pkr_rate:,.2f} PKR",
        f"  • NET LOSS:   Rs. {net_s9*0.35*pkr_rate:+,.2f} PKR ({net_s9:+.2f}%)  --> Wallet: Rs. {(35.0+net_s9*0.35)*pkr_rate:,.2f} PKR",
        "",
        f"[SCENARIO 10] WORST CASE: Shallow Fill (Rs. {17.5*pkr_rate:,.2f}) -> Direct Stop Loss Hit ({_fmt_num(p_sl)})",
        f"  • Gross: Rs. {gross_s10*0.35*pkr_rate:+,.2f} PKR | Fees: -Rs. {fee_s10*0.35*pkr_rate:,.2f} PKR",
        f"  • NET LOSS:   Rs. {net_s10*0.35*pkr_rate:+,.2f} PKR ({net_s10:+.2f}%)  --> Wallet: Rs. {(35.0+net_s10*0.35)*pkr_rate:,.2f} PKR",
        "================================================================================",
    ]
    return "\n".join(lines)
