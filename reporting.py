...
Renders both:
  ...
  3. 15-DAY STRATEGY EVALUATION & FAILURE DIAGNOSIS REPORT
...
def _cumulative_progress_section(end_dt: datetime) -> str:
    """
    Day 1 (start date) se लेकर 'end_dt' tak ka complete cumulative rolling progress stats.
    """
...
def generate_report(period_label: str, start_dt: datetime, end_dt: datetime, include_cumulative: bool = True) -> str:
    ...
    lines.append(f"Coins Scanned (Dynamic Universe): {coins_scanned}")
...
def generate_welcome_email() -> str:
    lines = [
        ...
        "",
        "Each report contains both Current Period Activity and Cumulative Rolling Progress.",
        "=" * 55,
    ]
    return "\n".join(lines)
