...
def resolve_pending_zones(exchange, coin: str, timeframe: str):
    """
    Chronological trade resolution against new closed candles.
    Fixes timestamp comparison string bug by parsing timestamps into tz-aware pd.Timestamp objects.
    """
...
def process_coin_timeframe(exchange, coin: str, timeframe: str, start_datetime: str, df_btc: pd.DataFrame | None = None):
    """
    Replays new candles chronologically for a single coin/timeframe pair.
    """
    tf_cfg = config.TF_SETTINGS[timeframe]
    ...
    for i in new_indices:
        ...
        result = analyze(coin, timeframe, df_slice, df_daily_slice, df_intermediate_slice, prev_state, df_btc_slice)
...
def live_check(exchange, coin: str, timeframe: str, df_btc: pd.DataFrame | None = None):
    tf_cfg = config.TF_SETTINGS[timeframe]
    ...
    return analyze(coin, timeframe, df, df_daily, df_intermediate, None, df_btc)
...
def live_check_all():
    ...
    try:
        ...
        logger.warning(f"Could not fetch BTC 1H data for regime check: {e}")

    qualifying = []
    for coin in coins:
        for timeframe in config.TIMEFRAMES:
            try:
                result = live_check(exchange, coin, timeframe, df_btc)
...
def scan_once():
    ...
    try:
        ...
        logger.warning(f"BTC 1H data fetch warning: {e}")
    ...
    for coin in coins:
        for timeframe in config.TIMEFRAMES:
            try:
                q_count, r_count, q_list = process_coin_timeframe(exchange, coin, timeframe, start_datetime, df_btc)
    ...
    db.insert_scan_log(scan_time, len(coins), zones_qualified_count, zones_rejected_count)
